...
// Dependencies contains all the dependencies needed for route initialization
...
func InitializeRoutes(cfg *config.Settings, r *gin.Engine, dep Dependencies) {
	...
	// Legacy WebSocket endpoint for comparison/transition
	ws := r.Group("/ws")
	{
		ws.GET("/legacy", handleLegacyWebSocket)
	}
}
...
func handleLegacyWebSocket(c *gin.Context) {
